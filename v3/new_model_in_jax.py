...
class TopBranch(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray, is_training: bool) -> jnp.ndarray:
        x = hk.Conv2D(32, (5, 5), stride=(1, 3))(x)
        x = hk.BatchNorm(decay_rate=0.9, create_scale=True, create_offset=True)(x, is_training=is_training)
        x = jax.nn.relu(x)
        return x
...
class YpBranch(hk.Module):
    def __init__(self):
        super().__init__()
    def __call__(self, x: jnp.ndarray, is_training: bool) -> jnp.ndarray:
        x = hk.Conv2D(16, (5, 5))(x)
        x = hk.BatchNorm(decay_rate=0.9, create_scale=True, create_offset=True)(x, is_training=is_training)
        x = jax.nn.relu(x)
        x = hk.Conv2D(8, (3, 39))(x)
        x = hk.BatchNorm(decay_rate=0.9, create_scale=True, create_offset=True)(x, is_training=is_training)
        x = jax.nn.relu(x)
        x = hk.Conv2D(1, (5, 5))(x)
        x = jax.nn.sigmoid(x)
...
class YnBranch(hk.Module):
    def __init__(self):
        super().__init__()
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        x = hk.Conv2D(32, (7, 7), stride=(1, 3))(x)
        x = jax.nn.relu(x)
        x = hk.Conv2D(1, (7, 3))(x)
        x = jax.nn.sigmoid(x)
        return x
...
class PosteriorgramModel(hk.Module):
    ...
    def __call__(self, audio: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        ...
        preprocessed = CQTHarmonicStacking()(audio)
